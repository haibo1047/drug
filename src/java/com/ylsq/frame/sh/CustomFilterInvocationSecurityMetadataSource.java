...
public class CustomFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
	...
	public final Collection<ConfigAttribute> lookupAttributes(String url) {
        ...
        SecuMenu menu = secuMenuService.findMenuByUrl(url);
        if(menu != null){
        	configAttrList = new ArrayList<ConfigAttribute>();
        	List<String> authorityList = secuRoleMenuService.findRoleListByMenuId(menu.getId());
        	for(String at : authorityList){
        		configAttrList.add(new SecurityConfig(at));
        	}
        }
